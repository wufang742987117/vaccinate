...
<script type="text/javascript">
	$(function(){
		...
		//数据回显
		var ovMonth = '${fns:toJson(expChildBaseInfoVo.ovmonth)}';
		if(ovMonth && ovMonth != "\"\""){
			$("#ovmonth").selectpicker('val',JSON.parse(ovMonth));
		}
		...
		if('${ExpChildBaseInfoVo.checkEx}' == '1'){
			$("#checkEx").prop("checked",true);
			$("#checkEx").parent().addClass("checked");
		}else{
			$("#checkEx").attr("checked",false);
			$("#checkEx").parent().removeClass("checked");
		} 
	});
	...
	$(document).ready(function() {
		$("#btnExport").click(function(){
			...
			var temp = $("#searchForm").attr("action");
				$("#searchForm").attr("action","${ctx}/child_baseinfo/overduevacc/export");
				$("#searchForm").submit();
...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/child_baseinfo/overduevacc">逾期未种</a></li>
	...
	<div class="ibox">
		<div >
			<form:form id="searchForm" modelAttribute="expChildBaseInfoVo"
				action="${ctx}/child_baseinfo/overduevacc/sele"
				...
				<div class="form-group" >
					<div class="input-group">
						<span class="input-group-addon gray-bg text-right">未种月限:</span>
						 <select title="请选择" id="ovmonth" name="ovmonth" class="form-control w155" data-live-search="false">
							<option value="1" >一个月</option><option value="2">二个月</option>
							<option value="3">三个月</option><option value="4">四个月</option>
							<option value="5">五个月</option><option value="6">六个月</option>
							<option value="7">七个月</option><option value="8">八个月</option>
							<option value="9">九个月</option><option value="10">十个月</option>
							<option value="11">十一个月</option><option value="12">十二个月</option>
                         </select> 
					</div>
				</div>
				<div class="form-group" >
					<div class="input-group">
						<span class="input-group-addon gray-bg text-right">现管理单位:</span>
						 <select title="请选择" id="officeinfo" name="officeinfo"  class="form-control w155" data-live-search="false">
                             <option value="0">所有</option>
                             <option value="1">本点管理</option>
                           <!--    <option value="2" class="situation" >非本点管理</option>  -->
                         </select> 
					</div>
				</div>
				<div class="form-group" >
