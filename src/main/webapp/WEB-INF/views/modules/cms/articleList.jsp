	...
	<script type="text/javascript">
		function viewComment(href){
			...
			parent.layer.open({
				type: 2 //此处以iframe举例
				,title: '查看评论'
				,area: [$(top.document).width()-220+'px',$(top.document).height()-120+'px']
				,shade: 0.3
				,content: href
				,btn: ['关闭']
						...
						<td>
							...
							<shiro:hasPermission name="cms:article:edit">
								...
								<shiro:hasPermission name="cms:article:audit">
									<a href="${ctx}/cms/article/delete?id=${article.id}${article.delFlag ne 0?'&isRe=true':'&isRe=false'}&categoryId=${article.category.id}" onclick="return confirmx('确认要${article.delFlag ne 0?'发布':'删除'}该文章吗？', this.href)" >${article.delFlag ne 0?'发布':'删除'}</a>
								</shiro:hasPermission>
							</shiro:hasPermission>
						</td>
					</tr>
