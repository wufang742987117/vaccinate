		...
		<div >
			<table id="contentTable" border="1" style="border:solid 1px black;border-collapse:collapse;text-align:center;font-size:22px;margin: 0 auto 30px auto; width:860px;">
				...
				<c:forEach items="${bsList}" var="i">
					<c:forEach items="${i}" var="bsHepatitisBCheckinStock">
						<tr>
							<c:if test="${bsHepatitisBCheckinStock.leng != 0}">
								<td rowspan="${bsHepatitisBCheckinStock.leng}">
									${bsHepatitisBCheckinStock.vaccineName}
								</td>
							</c:if>
							<td>
								${bsHepatitisBCheckinStock.batch}
							</td>
							<td>
								${bsHepatitisBCheckinStock.manufacturer}
							</td>
							<td>
								${bsHepatitisBCheckinStock.storeNum2}
							</td>
							<td>
								${bsHepatitisBCheckinStock.storeNum - bsHepatitisBCheckinStock.storeNum2}
							</td>
							<td>
								${bsHepatitisBCheckinStock.storeNum}
							</td>
						</tr>
					</c:forEach>
				</c:forEach>
