...
<ol id="${input}Preview"></ol><c:if test="${!readonly}"><a href="javascript:" onclick="${input}FinderOpen();" class="btn btn-white btn-sm">${selectMultiple?'添加':'选择'}</a>&nbsp;<a href="javascript:" onclick="${input}DelAll();" class="btn btn-white btn-sm">清除</a></c:if>
<script type="text/javascript">
	...
	function ${input}Preview(){
		...
		if ($("#${input}Preview").text() == ""){
			$("#${input}Preview").html("<li style='list-style:none;padding-top:5px;'>暂未选择</li>");
		}
	}
	${input}Preview();
</script>