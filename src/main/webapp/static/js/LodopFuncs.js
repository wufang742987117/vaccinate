...
function getLodop(oOBJECT,oEMBED){
    ...
    var strCLodopInstall="<br><font color='#FF00FF'>CLodop云打印服务(localhost本地)未安装启动!点击这里<a href='http://www.hbepi.com.cn/resource/CLodop_Setup_for_Win32NT_3.025.exe' target='_self'>执行安装</a>,安装后请刷新页面。</font>";
    var strCLodopUpdate="<br><font color='#FF00FF'>CLodop云打印服务需升级!点击这里<a href='http://www.hbepi.com.cn/resource/CLodop_Setup_for_Win32NT_3.025.exe' target='_self'>执行升级</a>,升级后请刷新页面。</font>";
    var LODOP;
    try{
        ...
        if (needCLodop()) {
            ...
            if (!LODOP) {
            	if (isIE) document.write(strCLodopInstall); 
            		...
            		console.error(strCLodopInstall);
            		$("body").before("<div class='printError'>" + strCLodopInstall + "</div>");
//            		document.documentElement.innerHTML=strCLodopInstall+document.documentElement.innerHTML;
            	...
            	if (CLODOP.CVERSION<"2.0.9.0") { 
            		if (isIE) document.write(strCLodopUpdate); 
                		...
                		$("body").before("<div class='printError'>" + strCLodopUpdate + "</div>");
//            			document.documentElement.innerHTML=strCLodopUpdate+document.documentElement.innerHTML;
            ...
            if ((LODOP==null)||(typeof(LODOP.VERSION)=="undefined")) {
                 if (navigator.userAgent.indexOf('Chrome')>=0){
            		...
            		$("body").before("<div class='printError'>" + strHtmChrome + "</div>");
//                     document.documentElement.innerHTML=strHtmChrome+document.documentElement.innerHTML;
                 }
                 if (navigator.userAgent.indexOf('Firefox')>=0){
          			console.error(strHtmFireFox);
            		$("body").before("<div class='printError'>" + strHtmFireFox + "</div>");
//                     document.documentElement.innerHTML=strHtmFireFox+document.documentElement.innerHTML;
                 }
                 if (is64IE) document.write(strHtm64_Install); 
                 else if (isIE) document.write(strHtmInstall); 
                 else{
           			console.error(strHtmInstall);
            		$("body").before("<div class='printError'>" + strHtmInstall + "</div>");
...
//        		document.documentElement.innerHTML=strCLodopUpdate+document.documentElement.innerHTML; 
       			...
       			console.error(strHtmUpdate);
        		$("body").before("<div class='printError'>" + strHtmUpdate + "</div>");
